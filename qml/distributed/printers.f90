...
contains
    ...
    subroutine print_version
        ...
        write(stdout, "(a)") "QML"
        write(stdout, "(a)")
        write(stdout, "(a)") "Source Code: github.com/qmlcode/qml"
        write(stdout, "(a,x,a)") "Version Tag:", ""
        write(stdout, "(a,x,a)") "Commit Hash:", ""
        write(stdout, "(a)")
        write(stdout, "(a)") "Cite:"
        write(stdout, "(a)") "See log file for the calculation specific reference"
        write(stdout, "(a)")
        write(stdout, "(a)") "Anything else? Read more at www.qmlcode.org"
        write(stdout, "(a)")
    ...
    subroutine print_warning(msg, subrout)
        ...
        write(stderr, "(3a)") 'qml warning: ', subrout, msg
    ...
    subroutine print_triangular(matrix, rows, accuracy) ! tri-shape, matrix, accuracy
        ...
        do i = 1, rows
            idx = (i*i+i)/2
            do j = idx-i+1, idx
                write(stdout, "(100f12.6)", advance='no')  matrix(j)
            end do
            write(stdout, "(a)")
        end do
